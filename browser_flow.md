...
- **全体フロー図**：URL入力から画面表示までの10段階をASCIIアートで俯瞰
- **具体例**：`<p class="foo">hello</p>` が各段階でどんなデータに変わるかを追う
- **各段階の詳細**：[1]〜[10] それぞれのファイル・処理内容・ポイントを解説
- **全体のオーケストレーション構造**：`Browser` / `Page` / `WasabiUI` の関係と呼び出し順
